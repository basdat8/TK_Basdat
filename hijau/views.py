...
@login_required
@require_POST
def add_catatan_medis(request, id):
...
# TODO: ADD DATE
def delete_catatan_medis(request, id):
    with connection.cursor() as cursor:
        cursor.execute("DELETE FROM catatan_medis WHERE id_hewan = %s", [id])
    return redirect('list_catatan_medis')

@login_required
@require_POST
def edit_catatan_medis(request, id):
    username_dh = request.user.username
    with connection.cursor() as cursor:
